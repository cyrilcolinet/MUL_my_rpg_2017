...
static void display_stat(data_t *data, sfVector2f pos, int i, int id)
{
	...
	if (id == 0)
		str = my_itoa(data->fight[data->id]->enemy[i]->hp);
	else if (id == 1)
		str = my_itoa(data->fight[data->id]->enemy[i]->dmg);
	if (id == 2)
		str = my_itoa(data->fight[data->id]->enemy[i]->armor);
	else if (id == 4)
		str = my_itoa(i + 1);
...
static void display_name_frame(data_t *data, sfVector2f coord, int i)
{
	...
	sfText_setPosition(data->text, pos);
	sfText_setString(data->text, "Enemy");
	sfRenderWindow_drawText(data->window, data->text, NULL);
	pos.x += 70;
	...
	sfRectangleShape_setPosition(
		data->fight[data->id]->enemy[i]->frame, pos);
	sfRenderWindow_drawRectangleShape(data->window,
			data->fight[data->id]->enemy[i]->frame, NULL);
}
...
void display_enemy_stats(data_t *data, sfVector2f coord)
{
	...
	if (data->fight[data->id]->number_enemy < 6)
		coord.x += 80;
